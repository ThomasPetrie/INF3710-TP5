...
CREATE TABLE Rapport (
    ...
    FOREIGN KEY (idobservateur) REFERENCES Observateur(idobservateur) ON DELETE SET NULL
);
...
CREATE TABLE Amateur (
    ...
    FOREIGN KEY (nomcommunaute) REFERENCES Communaute(nomcommunaute) ON DELETE CASCADE,
    FOREIGN KEY (idobservateur) REFERENCES Observateur(idobservateur) ON DELETE CASCADE
);
...
CREATE TABLE Observation (
    ...
    FOREIGN KEY (idobservateur) REFERENCES Observateur(idobservateur) ON DELETE CASCADE,
    FOREIGN KEY (nomscientifique) REFERENCES Especeoiseau(nomscientifique) ON DELETE CASCADE
);
...
CREATE TABLE Resider (
    ...
    FOREIGN KEY (nomscientifique) REFERENCES Especeoiseau(nomscientifique) ON DELETE CASCADE,
    FOREIGN KEY (nomzone) REFERENCES Zonegeographique(nomzone) ON DELETE CASCADE
);